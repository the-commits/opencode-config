...
## Prerequisite: Check Availability
Before using these tools, verify the MCP server is loaded (e.g., check if `search_graph` is in your available tools). If the MCP is not loaded, fall back to standard `grep`/`glob`/`read` tools. Do not attempt to call MCP tools if they are not listed.

## Priority Order
1. `search_graph` — find functions, classes, routes, variables by pattern (BM25, regex, or semantic vector search)
2. `trace_path` — trace who calls a function, what it calls, data flow, or cross-service links
3. `get_code_snippet` — read specific function/class source code (use `include_neighbors` for context)
4. `query_graph` — run Cypher queries for complex patterns, dead code, and hotspot detection
5. `get_architecture` — high-level project summary (languages, packages, routes, clusters)
6. `detect_changes` — map git diff to affected symbols and blast radius
7. `get_graph_schema` — explore available node/edge types and properties
8. `search_code` — graph-augmented grep over indexed files
9. `manage_adr` — CRUD for Architecture Decision Records
10. `ingest_traces` — ingest runtime traces to validate HTTP_CALLS edges

## Indexing & Project Management
- `index_repository`: Index a repo (modes: full, moderate, fast, cross-repo-intelligence). Use `persistence=true` to write a shared artifact.
- `list_projects`: List all indexed projects with node/edge counts.
- `index_status` / `delete_project`: Check status or remove a project.

## When to fall back to grep/glob
- Searching for string literals, error messages, config values
- Searching non-code files (Dockerfiles, shell scripts, configs)
- When MCP tools return insufficient results or the MCP is not loaded

## Common Workflows & Examples
- **Find a handler and its callers**:
  1. `search_graph(name_pattern=".*OrderHandler.*")`
  2. `trace_path(function_name="OrderHandler", direction="inbound", mode="calls")`
- **Read source code**: `get_code_snippet(qualified_name="pkg/orders.OrderHandler")`
- **Data flow tracing**: `trace_path(function_name="process", mode="data_flow", parameter_name="userInput")`
- **Cross-service tracing**: `trace_path(function_name="publishEvent", mode="cross_service")`
- **Semantic search**: `search_graph(semantic_query=["send","pubsub","publish"])`
- **Impact analysis**: `detect_changes(scope="uncommitted")`
- **Find performance hotspots**:
  `query_graph(query="MATCH (f:Function) WHERE f.transitive_loop_depth >= 3 OR f.linear_scan_in_loop >= 1 RETURN f.name, f.transitive_loop_depth ORDER BY f.transitive_loop_depth DESC")`
- **Find dead code**:
